  ...
  <div  >
    ...
    <div>
      <div>
        ...
        <select name="grade" id="gradeSelect" required v-model="this.gradeEnter">
      ...
      <my-button @click="gradeAffixing(this.dishKeeper)"> Подтвердить </my-button>
      </div>
    </div>
    </my-dialog> 
  </div>  
...
data() {
    return {
       triedDishes: [
        ...
        {name: 'Греческий салат', grade: 8, id: 2,
       ...
       unTriedDishes: [
        ...
        {name: 'Сельдь под шубой', grade: '', id: 1,
...
methods: {
  addDish() {this.dialogOfAdd = !this.dialogOfAdd},
  ...
  addDish2(newArray) {
   ...
   dishTransfer(dishKeeper) {
     this.unTriedDishes = this.unTriedDishes.filter(m => m.id !== dishKeeper.id)
